...
const [ selected,setSelecte] = useState(null)
const onBookHandler = ()=>{
    if(!selected){
                ...
                <div className='flex items-center gap-6 text-sm mt-5'>
                    ...
                    <span className='grid grid-cols-3 md:flex flex-wrap md:max-w-lg gap-4'>
                        {Object.keys(dateTime).map((date)=>(
                            <button onClick={()=>setSelecte(date)} className={`flex flex-col items-center justify-center h-14 w-14 aspect-square rounded cursor-pointer border border-primary/70 ${selected===date ? "bg-primary text-white":"border border-primary/70"}`} key={date}>
